...
trait IDeVote<ContractState> {    
    ...
    fn get_person_id_number(self: @ContractState) -> felt252;
...
mod DeVote {
    ...
    impl DeVoteImpl of IDeVote<ContractState> {
        ...
        fn create_new_person(ref self: ContractState, id_number: felt252) {
            ...
            person.wallet_id.write(wallet_id);
            person.role.write(0);
            self.emit(PersonAdded { wallet_id: person.wallet_id.read(), id_number: person.id_number.read(), role: person.role.read() });
        }
